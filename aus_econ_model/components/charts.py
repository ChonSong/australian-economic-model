...
def _apply_theme(fig: go.Figure) -> go.Figure:
    fig.update_layout(
        ...
        legend=dict(
            font=dict(color=THEME["font_color"]), bordercolor=THEME["grid_color"]
        ),
        hoverlabel=dict(font=dict(color=THEME["font_color"]), bgcolor="#1e1e1e"),
        xaxis=dict(
    ...
    for ann in fig.layout.annotations:
        if ann is not None:
            ann.font.color = (
                ann.font.color if ann.font and ann.font.color else THEME["font_color"]
            )
    return fig
...
def plot_time_series(sol, width=None, height=400):
    ...
    for row in [1, 2, 3]:
        fig.add_hline(
            y=0, line_color=THEME["zeroline_color"], line_width=1, row=row, col=1
        )
...
def plot_parameter_sensitivity(
    sol_base, params_list, sols_list, width=None, height=400
):
    """
