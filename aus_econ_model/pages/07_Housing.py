...
hp.initial_price_income = st.sidebar.slider("Initial P/I Ratio", 4.0, 10.0, 6.5, 0.5)

st.sidebar.subheader("Mortgage & LVR")
hp.mortgage_share_of_debt = st.sidebar.slider(
    "Mortgage Share of Debt", 0.2, 0.6, 0.45, 0.05
)
hp.initial_lvr = st.sidebar.slider("Initial LVR", 0.3, 0.8, 0.60, 0.05)
hp.lvr_cycle_sensitivity = st.sidebar.slider(
    "LVR Cycle Sensitivity", 0.0, 0.3, 0.10, 0.02
)

st.sidebar.subheader("Construction")
hp.base_construction_gdp = (
    st.sidebar.slider("Base Construction (% GDP)", 3, 10, 6, 1) / 100
)
hp.construction_elasticity = st.sidebar.slider(
    "Construction Price Response", 0.0, 0.5, 0.20, 0.05
)
...
fig.add_trace(
    go.Scatter(x=hs["t"], y=hs["mortgage_gdp"], name="Mortgage/GDP"), row=1, col=2
)
fig.add_trace(go.Scatter(x=hs["t"], y=hs["lvr"], name="LVR"), row=1, col=3)
fig.add_hline(
    y=0.80, line_dash="dash", line_color="red", annotation_text="LVR cap", row=1, col=3
)

# Row 2
fig.add_trace(
    go.Scatter(x=hs["t"], y=hs["rent_to_income"], name="Rent/Income"), row=2, col=1
)
fig.add_trace(
    go.Scatter(x=hs["t"], y=hs["rental_yield"], name="Rental Yield"), row=2, col=2
)
fig.add_hline(
    y=hp.rental_yield_target,
    line_dash="dash",
    line_color="gray",
...
fig.add_trace(
    go.Scatter(x=hs["t"], y=hs["construction_gdp"], name="Construction"), row=2, col=3
)
fig.add_hline(
    y=0.06, line_dash="dash", line_color="gray", annotation_text="Base", row=2, col=3
)

# Row 3
fig.add_trace(
    go.Scatter(x=hs["t"], y=hs["housing_wealth_gdp"], name="Wealth/GDP"), row=3, col=1
)
fig.add_trace(
    go.Scatter(
        x=hs["t"], y=hs["mortgage_service"], name="Service Ratio", fill="tozeroy"
    ),
    row=3,
    col=2,
)
fig.add_trace(
    go.Scatter(x=hs["t"], y=hs["dwelling_stock"], name="Dwelling Stock"), row=3, col=3
)
fig.add_hline(
    y=1.0,
    line_dash="dash",
...
scenario = st.radio(
    "Select scenario",
    ["Baseline", "Credit Boom", "Supply Shock", "Rate Hike", "Immigration Surge"],
    horizontal=True,
)

if scenario == "Credit Boom":
    test_hp = HousingParams(
        credit_flow_elasticity=hp.credit_flow_elasticity * 2,
        fundamental_price_income=hp.fundamental_price_income,
    )
    label = "Credit Boom (2× elasticity)"
elif scenario == "Supply Shock":
    test_hp = HousingParams(
        structural_trend=hp.structural_trend + 0.005,
...
if scenario != "Baseline":
    ...
    fig2.add_trace(
        go.Scatter(x=hs["t"], y=hs["price_to_income"], name="Baseline"), row=1, col=1
    )
    fig2.add_trace(
        go.Scatter(
            x=hs_test["t"],
            y=hs_test["price_to_income"],
            name=label,
            line=dict(dash="dash"),
        ),
        row=1,
    ...
    fig2.add_trace(
        go.Scatter(x=hs["t"], y=hs["mortgage_gdp"], name="Baseline"), row=1, col=2
    )
    fig2.add_trace(
        go.Scatter(
            x=hs_test["t"],
            y=hs_test["mortgage_gdp"],
            name=label,
            line=dict(dash="dash"),
        ),
        row=1,
    ...
    fig2.add_trace(
        go.Scatter(x=hs["t"], y=hs["housing_wealth_gdp"], name="Baseline"), row=1, col=3
    )
    fig2.add_trace(
        go.Scatter(
            x=hs_test["t"],
            y=hs_test["housing_wealth_gdp"],
            name=label,
            line=dict(dash="dash"),
        ),
        row=1,
...
st.caption(
    "Sources: RBA Statistical Tables (D2, E1, G1), ABS National Accounts, CoreLogic data patterns, Professor Steve Keen's credit-driven asset pricing framework."
)
