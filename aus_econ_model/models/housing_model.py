...
def simulate_housing(core_sol, params: Optional[HousingParams] = None) -> dict:
    ...
    lag_buf = np.full(int(max(params.construction_lag * 5, 5)), params.base_construction_gdp)
    ...
    for i in range(1, n):
        ...
        credit_growth_smoothed = (1.0 - params.credit_flow_persistence) * d_growth + params.credit_flow_persistence * credit_growth_smoothed
        
        # Income growth
        inc_growth = np.clip((income[i] - income[i-1]) / max(income[i-1], 0.01), -0.3, 0.3)
        
        # Fundamental value (drifts upward)
        fund_val = params.fundamental_price_income * (1.0 + params.structural_trend) ** t[i]
        ...
        # --- Dwelling stock ---
        nd = cgdp[i] * 0.15
        ds[i] = np.clip(ds[i-1] * (1.0 - params.dwelling_depreciation * dt) + nd * dt, 0.5, 2.0)
    ...
    return {
        't': t, 'price_to_income': pti,
        'mortgage_gdp': mg, 'lvr': lvr_a,
        'mortgage_service': ms,
        'rent_to_income': rti, 'rental_yield': ry,
        'construction_gdp': cgdp,
        'dwelling_stock': ds,
        'housing_wealth': hw, 'housing_wealth_gdp': hw,
